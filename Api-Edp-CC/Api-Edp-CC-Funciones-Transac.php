...
///Funcion que inserta valores de detalles de panel de sabores  falta
...
///Funcion que anula valores de detalles de analisis fisico
function Anular_detalles_Remuestra_Fisica($p_db, $p_empresa, $p_sucursal, $p_detalle, $p_usuario)
{
    //// Muestras panel
    //$gl_muestras_sabor
    ///// fin muestras cabezas
    ...
    if ($p_detalle == null) {
        $code = 204;
        $message = 'Ingrese codigo de Detalle';
        ...
        if ($p_usuario == null) {
            ...
            //////Ingreso de datos
            try {
                $query = $p_db->prepare("  UPDATE edpkedrf
                                                                SET kedrf_est_kedrf = 'AN',
                                                                    kedrf_fes_anula = current,
                                                                    kedrf_usu_anula = $p_usuario,
                                                                    kedrf_obs_anula = 'Anulado desde App-Aseg-CC',
                                                                    kedrf_ing_appcc = '1'
                                                                WHERE ( edpkedrf.kedrf_cod_kedrf = $p_detalle ) AND
                                                                    ( edpkedrf.empr_cod_empr = $p_empresa ) AND
                                                                    ( edpkedrf.sucu_cod_sucu = $p_sucursal )
                          ");
                $query->execute();
                ...
                if (!$query) {
                    $code = 204;
                    $message = 'Error al anular el dato';
                    ...
                    $code = 200;
                    $message = 'Dato Anulado Correctamente';
...
//// Fin Funcion que anula valores de analisis fisico

///Funcion que anula valores de detalles de analisis fisico
function Anular_detalles_Remuestra_Cabezas_Cargadas($p_db, $p_empresa, $p_sucursal, $p_detalle, $p_usuario)
{
    //// Muestras panel
    //$gl_muestras_sabor
    ///// fin muestras cabezas
    ...
    if ($p_detalle== null) {
        ...
        if ($p_usuario == null) {
            ...
            //////Ingreso de datos
            try {
                $query = $p_db->prepare("  UPDATE edpkedcc
                                SET edpkedcc.kedcc_est_kedcc = 'AN',
                                    kedcc_fes_anula = current,
                                    kedcc_usu_anula = $p_usuario,
                                    kedcc_obs_anula = 'Anulado desde App-Aseg-CC',
                                    kedcc_ing_appcc = '1'
                            WHERE ( edpkedcc.kedcc_cod_kedcc = $p_detalle ) AND
                                    ( edpkedcc.empr_cod_empr = $p_empresa ) AND
                                    ( edpkedcc.sucu_cod_sucu = $p_sucursal )
                          ");
                $query->execute();
